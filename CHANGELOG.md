...
## [0.9.8] - 2026-08-15 (License Expiry, Server License Duration)
### Added
- **License expiry date:** When generating a license in the Admin Tool, you can now set a duration (Days / Weeks / Months / Years / Unlimited). Expired licenses are automatically rejected by the server.
- **Expiry display in Admin Tool:** License list shows "Valid until DD.MM.YYYY" or "Unlimited". Expired licenses are marked in red.
- **Expiry display in LMU RC App:** Settings page shows "Valid until DD.MM.YYYY" or "Unlimited" next to the license version.
- **Server migration:** Existing databases automatically get the `expires_at` column added.

### Changed
- **License validation:** Server now checks `expires_at` on both `validate-license` and `activate-license` endpoints. Expired licenses are rejected immediately.
- **LMU RC App offline grace:** Expired licenses are blocked immediately (no 14-day grace period for expired licenses).
...
- **Color-specific cooldown:** RED (10s), YELLOW (30s), WHITE (30s) â€“ each color has its own cooldown timer. A RED incident no longer blocks YELLOW or WHITE detection.
...
### Changed
- **RED cooldown reduced:** From 30s to 10s â€“ so a second collision shortly after the first is now detected.
...
- **Release process:** `release.ps1` is now the single entry point for all releases. No more GitHub Workflows needed â€“ build, sign, upload, latest.json, and Discord all run locally.
...
### Fixed
- **`release.ps1` `Test-Path` syntax:** `Test-Path $x -and Test-Path $y` was invalid PowerShell syntax â€“ fixed to `(Test-Path $x) -and (Test-Path $y)`.
- **`release.ps1` `$ErrorActionPreference`:** `"Stop"` mode caused `cargo` Info messages to throw errors â€“ now set to `"Continue"` during build.
- **Discord 403 Forbidden:** Added `User-Agent` header to webhook requests â€“ Discord blocks `Python-urllib/3.x` without it.

## [0.9.5] - 2026-08-10 (Release Repo, Update Fix, Discord Notification, Server Backup, License Deactivation)
### Added
- **Release repo created:** `FOKALiS/LMU-Racecontrol-Releases` â€“ release artifacts only (no source code)
- **Discord release notification:** Workflow `.github/workflows/discord-notify.yml` â€“ automatic on release with formatted changelog
- **Server backup script:** `scripts/backup-server.ps1` â€“ automatic (Task Scheduler) + manual
- **License deactivation:** User can deactivate license (machine change) â€“ in Settings
...
- **Button "Request API Key":** In Settings â†’ Server Connection â€“ requests API key for license key (licensed Enterprise only)
- **Version in license heading:** "License Information â€“ Version: Enterprise XL (Server)"
...
### Fixed
- **Pipo Derani fix:** Introduced `MIN_STOPPED_SPEED_KMH = 0.5` â€“ vehicles at 0.0 km/h are no longer detected as stopped

### Changed
- **Discord webhook:** Formatting overhauled (driver, warning/penalty points, "N.A." for empty corner)
- **Settings:** Heading left-aligned (text-align: center â†’ left)
- **Folder icon (ðŸ“‚)** for LMU path selection restored
- **Archive:** Decision badge now opens InvestigationModal
- **Window title:** "LMU RACECONTROL â€“ The tool for race stewards"
...
- **Incident detection overhauled:**
  ...
  - YELLOW: Lap time >15%, position loss â‰¥3, FCY violation
...
- **Keyboard config fixed:**
  ...
  - OnceLock â†’ RwLock (reload now works multiple times)
- **File browser (ðŸ“‚)** for LMU installation path in Settings
...
### Changed
- **Player bar icons:** Emoji placeholders (â®âªâ–¶â©â­) replaced with real PNG icons (Slow Rewind, Rewind, Play, Forward, Slow Forward) in all three views (Driver Field, Incidents, Archive)
...
### Changed
- **Camera Helper removed:** The separate `camera-helper` process has been completely removed. Camera control now runs directly via `SendInput` with scancodes from the user's key bindings â€“ no external process needed.
...
- **Longer pauses between commands:** 200ms after mode switch, 500ms after time jump â€“ giving LMU enough time to process commands.
...
### Source
- Analysis of BCUK (Broadcast Control UK) on desktop confirmed: BCUK uses exactly the same REST API approach (`/rest/watch/focus/{name}`) â€“ the difference is the **order**: first replay mode + time jump, then camera.
- BCUK plugin JS (Stream Deck) shows: Camera control runs via `POST /api/control` with actions like `setTv`, `setNose`, `setCockpit`, `setOnboard`, `setOnCockpit`, `setOnDash`, `setOnRear` â€“ which are mapped to `/rest/watch/focus/{name}` internally by LMU.
- Version: 0.6.11 â†’ 0.6.12

## [0.6.11] - 2026-07-25 (REST API PUT Body Fix + Camera Key Fix)
### Fixed
- **All PUT requests failed (HTTP 400):** The LMU REST API requires an empty JSON body `{}` with `Content-Type: application/json` for PUT requests. Without a body, HTTP 400 was returned â€“ affecting: focus, camera, replaytime, switch_to_live/replay
- **Camera control now works reliably via REST API:** `/rest/watch/focus/TV`, `/rest/watch/focus/Onboard`, `/rest/watch/focus/Heli` and others confirmed via curl âœ… The endpoint `/rest/watch/focus/{name}/{group}/{advance}` is NOT supported by LMU (HTTP 400) and has been removed from the code.
...
- Version: 0.6.10 â†’ 0.6.11

## [0.6.7] - 2026-07-24 (Shared Memory, Connect/Disconnect, FCY +3 km/h Tolerance)
### Added
- **Shared Memory (rFactor 2/LMU):** Direct access to LMU Shared Memory (`Local\rFactor2SharedMemory`). Camera switching and vehicle focus now work **without window focus, keyboard simulation, or terminal flash** â€“ just like Broadcast Control UK, SimHub and other professional tools
- **Connect/Disconnect hover:** When connected, hovering the mouse shows "Disconnect from Server" (red) â€“ click disconnects
...
### Changed
- `shared_memory.rs`: New module â€“ writes camera values directly to LMU Shared Memory (group + camera ID)
...
- **Version:** 0.6.6 â†’ 0.6.7

## [0.6.6] - 2026-07-24 (Keyboard Control: Scancodes, AttachThreadInput for LMU Focus on Another Monitor)
### Fixed
- **Key presses now reliably reach LMU (not the Tauri app):** Scancodes via `KEYEVENTF_SCANCODE` instead of virtual key codes â€“ games use scancodes for their key bindings
...
- **Version:** 0.6.5 â†’ 0.6.6

## [0.6.5] - 2026-07-24 (Keyboard Control New: Win32 SendInput, No PowerShell Flash, Driver Field Sorting, Camera Selection Active)
### Fixed
- **Terminal window flash when focusing LMU eliminated:** PowerShell `AppActivate` replaced with native Win32 `FindWindowW`/`SetForegroundWindow` â€“ no more flashing terminal
...
- **Fonts (Michroma/Inter) now embedded locally:** Font files are stored as `.woff2` in `frontend/public/fonts/` and are built into the app â€“ no more Google Fonts network access. The app now works completely offline.
...
### Added
- **Camera control via keyboard simulation:** Camera buttons (TV, Helmet, Front, Rear, Top, Behind) now simulate F1-F6 key presses directly in LMU/rFactor2 â€“ works because the LMU REST API doesn't provide a camera endpoint
...
### Changed
- **Splashscreen design overhauled:** Logo enlarged (300px â†’ 380px), version number placed below logo, overall layout visually enhanced
- **Backend:** Keyboard simulation switched from `windows` crate to `enigo` crate (solves version conflicts with Tauri 2)
- **Version:** 0.5.4 â†’ 0.6.0 (all version numbers updated)

## [0.5.4] - Unreleased (Cam Control Right-Aligned)
### Changed
- Image Control + Cam Control now aligned as one unit to the right (same width as the Race Control row below) â€“ Image Control keeps its size and only moves as a whole to the right, Cam Control now ends exactly at the true right edge instead of having empty space after it
...
### Changed
- `KEYGEN_ACCOUNT` in `src-tauri/src/license.rs` changed from placeholder to the real Keygen account ID â€“ license validation is now actually functional (assuming the Keygen policy is configured as discussed: Node-locked, authentication strategy License/Mixed)

## [0.5.0] - Unreleased (License System)
### Added
- License requirement: without a valid license, only the splash screen (with license key input), "Help" and the website link are usable â€“ all other functions (Driver Field, Incidents, Archive, Settings, Connect to Server, FCY) are locked
...
### Important â€“ before the next build
- Replace the placeholder `KEYGEN_ACCOUNT` in `src-tauri/src/license.rs` with the real Keygen account slug â€“ without this step, every license validation will fail
...
- Update strategy: repository remains private for now, auto-update detection will work automatically once the project is set to "Public" â€“ until then, updates continue to be downloaded manually from the Releases page
...
- Build workflow now uses the official `tauri-apps/tauri-action`: cryptographically signs updates, automatically creates a versioned GitHub release (e.g. "v0.4.0") including the `latest.json` required by the updater â€“ replaces the previous provisional "latest" release
...
### Important
- Before the next build, the signing key pair must be generated and set up once (guide, step 0) â€“ otherwise the build will fail
...
- App icon is now automatically generated from `src-tauri/icons/icon-source.png` during build (all sizes/formats) â€“ just replace this one image to change the icon
...
- New help window (click "Help" in sidebar) with quick overview of usage â€“ text freely editable in `frontend/src/content/helpContent.ts`
...
- Red/Yellow/White status dots in Driver Field (crash suspicion / unusual pace anomaly / slow vehicle) â€“ see "Known Gaps"
- Discord webhook notification for every commission decision
- New "Settings" area (not included in mockup, but necessary): incident categories, decision options, Discord webhook, FCY parameters â€“ stored locally per installation
...
- SQLite persistence of all incidents including status workflow (Suspicion â†’ Under Review â†’ No Action / Penalty Imposed)
...
- No confirmed REST endpoint for damage/contact per opposing vehicle found â€“ automatic detection therefore works with pace/position anomalies instead of direct collision detection
